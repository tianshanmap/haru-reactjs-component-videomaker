...
export default function DragAndDropProfile({image_path,onComplete,onExit}) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const result = await api.getFilelist(image_path,".jpeg");

        // 1. Split the string into an array
        // 2. Use .map() to return an object for each item
        const fruitObjects = result.filter(x => !x.name.startsWith(".") && (x.name.endsWith(".jpeg") || x.name.endsWith(".jpg"))).map(x => {
      ...
      <div className={styles.drag_drop_container}>
        ...
        >
          ...
          {getItemsByColumn('left').map((item) => (
            ...
            >
              <img src={api.getViewEndPoint(item.id)} className={styles.drag_and_drop_img} width="100" height="50" />
            </div>
          ))}
        </div>
              {/* Column 2 */}
        ...
        >
          ...
          {getItemsByColumn('right').map((item) => (
            <div
              ...
              className={styles.drag_drop_item}
            >
              <img src={api.getViewEndPoint(item.id)} className={styles.drag_and_drop_img} />
            </div>
          ))}
        </div>
      </div>
